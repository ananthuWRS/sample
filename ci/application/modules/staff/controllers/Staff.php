...
class Staff extends MY_Controller
{
    ...
    public function updateTaskStatusDetails()
    {
        ...
        if ($this->task->validate($validation, 'formvalid')) {
            $curdate = date('Y-m-d H:i:s');
            if ($editid != "") {
                $checkDataEntered= $this->taskstatus->get_by(array('td_staff_id'=>$this->loggeduserid,
                 'td_task_id'=>$editid,'td_execution_date'=>date('Y-m-d', strtotime($task_execution_date))));
                if (!$checkDataEntered) {
                    $checkDateandStatus=$this->taskstatus->get_by(array('td_staff_id'=>$this->loggeduserid,
    ...
    public function editTaskStatusDetailsProcess()
    {
        ...
        if ($this->task->validate($validation, 'formvalid')) {
            ...
            if ($editid != "") {
                ...
                $editDataCheck= $this->taskstatus->get_by(array('td_staff_id'=>$this->loggeduserid,
